...
export default function SlotPicker({
  ...
  interviewer: any;
  interviewerCredits: number;
  userCredits: number;
}) {
  const router = useRouter();
  const dates = useMemo(() => generateDates({ daysAhead: DAYS_AHEAD }), []);
        ...
        <div className="bg-[#0f0f11] border border-white/10 rounded-2xl p-7 flex flex-col gap-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="text-right shrink-0">
              ...
              <p className="font-serif text-2xl leading-none bg-linear-to-br from-[#3AE4B2] to-[#22ff00] bg-clip-text text-transparent">
                {interviewerCredits}
          ...
          <div className="flex gap-2 overflow-x-auto pb-0.5 scrollbar-none -mx-1 px-1">
            {dates.map((date) => {
              const label = formatDateTab({ date: date.toISOString() });
              const active =
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`mt-0.5 ${
                      active ? "text-[#3AE4B2]/70" : "text-stone-700"
                    }`}
                  >
                    {label.bottom}
                  </span>
